...
medium_prep |>
  ...
  as.list() |>
  (function(l) 
    sprintf("%s: %s", names(l), map_chr(
      l, ~if(is.numeric(.x)) paste(signif(.x, 4)) else .x)) |>
    cat(sep = "\n")
  )()
```
...
medium_prep |>
  ...
  # plot
  select(buffer_with, x_TIC_consumed, matches("actual_(DIC|H2|pCO2|pH)")) |>
  pivot_longer(cols = c(starts_with("actual"))) |>
  mutate(panel = str_remove(name, "actual_")) |>
  ggplot() + 
  aes(1 - x_TIC_consumed, value, color = name) +
  geom_line() +
